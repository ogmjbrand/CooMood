...
// CooMood "fragrance mist" — adapted from a flowing-particle-sheet reference concept,
// recolored from teal/mint into the brand's near-black / gold / cherry palette.
const bgColor = "#120b06";
const flameColor = "#c8a96a";
const flameColor2 = "#8b1120";
const flameAmt = 0.22;
const atmoColor = "#e4cd9a";
const atmoCount = 300;
const atmoSize = 24;
const atmoSpeed = 1.0;
const colorLow = "#1a1006";
const colorHigh = "#c8a96a";
const opacity = 0.28;
const pointSize = 5.5;
...
export default function FlowWaveScene() {
  ...
  useEffect(() => {
    ...
    torusComposer.addPass(new UnrealBloomPass(new THREE.Vector2(1, 1), 0.22, 0.2, 0));
    torusComposer.addPass(new ShaderPass(CopyShader));

    const bloomComposer = new EffectComposer(renderer);
    bloomComposer.renderToScreen = false;
    bloomComposer.addPass(renderPass);
    bloomComposer.addPass(new UnrealBloomPass(new THREE.Vector2(1, 1), 0.4, 0.55, 0));
    ...
    // Scroll (scoped to this section's own bounds, not the whole document)
    let scrollTarget = 0;
    let scrollSmooth = 0;
    let scrollCurrent = 0;
    ...
    function resize() {
      ...
      atmoMaterial.uniforms.uRes.value.set(width * dpr, height * dpr);
      computeScrollTarget();
    }
    ...
    function frame() {
      ...
      camera.lookAt(
        mouse.x * parallax * 0.5,
        Lerp(0.0, 0.6, e),
        Lerp(lookStartZ, lookEndZ, e)
      );
      ...
      atmoMaterial.uniforms.uTime.value = t * atmoSpeed * 8.0;
    ...
    <section ref={sectionRef} className="relative" style={{ height: "260vh" }}>
      <div className="sticky top-0 h-screen w-full overflow-hidden">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
          <span className="mb-5 font-sans text-xs uppercase tracking-[0.35em] text-gold-light">
            The Ritual
          </span>
          <h2 className="font-display text-5xl leading-[1.05] text-cream sm:text-7xl">
            Fragrance, Suspended in Motion.
          </h2>
          <p className="mt-6 max-w-lg font-sans text-sm text-cream/60 sm:text-base">
            Every CooMood scent is built in layers, the way this room is built in light — top notes
            catching first, base notes settling last.
          </p>
        </div>
      </div>
    </section>
